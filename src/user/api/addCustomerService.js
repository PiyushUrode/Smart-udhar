...
export const useCustomerForm = () => {
        ...
        if (customerId) {
            const fetchCustomer = async () => {
                try {
                    ...
                    const res = await axiosClient.get(`/store-customer/findBy-id/${customerId}`, {
                        headers: { Authorization: `Bearer ${token}` },
                    });

                    if (res.data?.success) {
                        // Reset image to null so we don't accidentally send old image data
                        setFormData({ ...res.data.customer, image: null });
                        if (res.data.customer.image) {
                            const baseURL = axiosClient.defaults.baseURL;
                            setPreview(`${baseURL}/assets/uploadCustomerImage/${res.data.customer.image}`);
                        }
                    }
                } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // 🛑 FIX APPLIED: Only append fields that are NOT store_id/storeProfile_id 
            // to avoid sending duplicate IDs when updating.
            ...
            // ✅ Explicitly append the single, current IDs from AuthContext
            ...
            console.error(err);
            // Use server-provided error message or a generic one
            alert(err.response?.data?.message || err.message || "Error saving customer!");
        } finally {
            setLoading(false);
        }
    };
