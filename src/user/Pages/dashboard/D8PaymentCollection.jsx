...
import Button from "../../common/Button.jsx"; // 👈 Import custom Button component

const D8PaymentCollection = () => {
  // 1. State for the custom notification/pop-up component
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      setPopupType('error');
      setMessage("Failed to generate PDF receipt.");
    }
  };
  ...
  const handleGenerateReceipt = async () => {
    ...
    // Optional: Add a check for the confirmation checkbox here if needed
    // const isConfirmed = document.getElementById("confirmAmount").checked;
    // if (!isConfirmed) {
    //   setPopupType('error');
    //   setMessage("Please confirm the amount before generating the receipt.");
    //   return;
    // }

    setPopupType('processing');
    setMessage("Updating milestone status...");
    ...
    try {
      ...
      const res = await Invoice.updateMilestones(
        invoiceData._id,
        updatedMilestones
      );
      ...
      if (res.success) {
        ...
        setMessage(`Receipt successfully generated for ${fmtINR(selectedMilestone.amount)}!`);
      } else {
        handleCloseConfirmationModal();
        setPopupType('error'); // 👈 Error popup
        setMessage("❌ Failed to update milestone. Please try again.");
      }
    } catch (err) {
      console.error("Update milestone error:", err);
      handleCloseConfirmationModal();
      setPopupType('error'); // 👈 Error popup
      setMessage("❌ Error updating milestone due to a network or server issue.");
    }
  };
  ...
  return (
    <section className="w-full min-h-screen bg-[#f9f9ff] mt-5 px-4 md:px-10 py-6">
          ...
          <div className="flex flex-col gap-4 ">
            ...
            <div className="text-[#4B5563] font-robotoM leading-5 font-robotoR">
              ...
              <p className="text-sm font-robotoR">
                <span className="font-robotoM"> Address: </span> 123 MG Road,
                Bangalore, Karnataka 560001
              </p>
              <p className="text-sm font-robotoR">
                <span className="font-robotoM">Payment Mode:</span>{" "}
                {invoiceData?.paymentMode || "-"}
              </p>
              <p className="text-sm font-medium font-robotoR">
                <span className="font-robotoM">Next Payment:</span>{" "}
                {fmtDate(nextMilestone?.dueDate)}
              </p>
            </div>
          </div>

          <div className="w-full md:w-1/2 mt-4 md:mt-0 ">
            <h4 className=" font-robotoM text-black mb-2">
              Payment Milestones
            </h4>
            ...
            {milestones.map((m, idx) => {
              ...
              return (
                <tr key={m._id || idx} className="border-t border-gray-200">
                  ...
                  <td className="px-4 py-3">
                    {m.paymentMode || invoiceData?.paymentMode || "-"}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {status === "pending" ? (
                      <button
                        className="text-xs bg-blue-600 text-white px-3 py-1 rounded-md hover:bg-blue-700 transition-all duration-200"
                        onClick={() => handleMarkAsPaid(m)}
                      >
                        Mark as Paid
                      </button>
                      ...
                      <button
                        className="text-xs bg-gray-200 text-gray-500 px-3 py-1 rounded-full cursor-default"
                        disabled
                      >
                        Paid
                      </button>
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-300 p-6 mx-auto mt-5">
        <h4 className="text-base font-semibold text-gray-700 mb-5 border-b pb-2">
          Receipt Preview
        </h4>
        <div
          id="receiptDiv"
          className="bg-[#F9FAFB] border border-gray-300 rounded-md p-6 max-w-md mx-auto"
        >
          <h5 className="text-center font-bold text-gray-800 text-lg">
            PAYMENT RECEIPT
          </h5>
          ...
          <div className="space-y-2 text-sm text-gray-700">
            <div className="flex justify-between">
              ...
              <span>{fmtDate(invoiceData?.createdAt)}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between font-medium">
              ...
              <span className="text-green-700">{fmtINR(totalPaid)}</span>
            </div>
            <div className="flex justify-between font-medium">
              <span>Remaining:</span>
              <span className="text-yellow-800">{fmtINR(remainingAmount)}</span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-blue-600">
                {nextMilestone
                  ? `${nextMilestone.milestoneName} - ${fmtINR(
                      nextMilestone.amount
                    )} on ${fmtDate(nextMilestone.dueDate)}`
                  : "All Paid"}
              </span>
            </div>
      ...
      {/* 🟢 Custom Button Component for Pop-up Messages */}
      {popupType && (
        <Button
          type={popupType}
          message={message}
          onClose={() => {
            setPopupType(null);
            setMessage(""); // Clear message as well
          }}
        />
      )}
      
      {/* ⚠️ Existing Modal for Milestone Confirmation/Input */}
      {showConfirmationModal && selectedMilestone && (
